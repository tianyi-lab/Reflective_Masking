...
class FinetuneSolverBase(ABC):
    ...
    def configure_logger(self):
        ...
        if rank == 0:
            f_handler = logging.FileHandler(Path(self.args.output_dir) / "common.log")
            f_handler.setLevel(logging.INFO)
            handlers.append(f_handler)

        f_rank_handler = logging.FileHandler(Path(self.args.output_dir) / f"rank-{rank}.log")
        f_rank_handler.setLevel(logging.INFO)
        handlers.append(f_rank_handler)
    ...
    def get_args_parser(cls):
        ...
        parser.add_argument(
            "--no-save-intermediate-checkpoints",
            action="store_false",
            dest="save_intermediate_checkpoints",
            help="disable intermediate checkpoints and keep only final checkpoint",
        )
        parser.add_argument(
            "--save_optimizer",
            action="store_true",
        ...
        parser.add_argument(
            ...
            action="store_false",
            dest="save_optimizer",
            help="skip optimizer state when saving checkpoints",
        )
    ...
    @abstractmethod
    def _model_func(self, init_from: str) -> (nn.Module, Tokenizer | None):
    ...
    def resume(self, resume_path: str):
        ...
        def _load_optimizer():
            ...
            if opt_state_world_size == 0:
                self.logger.warning(
                    "No optimizer states found in %s; resume model weights with a fresh optimizer.",
                    resume_path,
                )
                return
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            should_save_final = epoch + 1 == self.args.epochs
            should_save_epoch = self.args.save_intermediate_checkpoints and (epoch % self.args.save_interval == 0)
            if should_save_epoch or should_save_final:
                util.ckpt.save(
