...
def _read_model_type(model_name_or_path: str) -> str | None:
...
class DreamHistoryConfig(DreamConfig):
    model_type = "dream_history"
...
class DreamSyntheticRevisionHistoryModel(DreamModel):
    config_class = DreamHistoryConfig
    modules_to_save = {"history_output_norm"}

    def __init__(self, config: DreamHistoryConfig):
        super().__init__(config=config)
        self.history_output_norm = nn.LayerNorm(config.hidden_size)
        self.history_output_norm.reset_parameters()
    ...
    def _history_residual(
        ...
        *,
        ...
        token_mask = history_token_mask.to(
    ...
    def save_pretrained(self, save_directory, *args, **kwargs):
        ...
        base_payload["model_type"] = "Dream"
        base_payload["architectures"] = ["DreamModel"]
    ...
    def from_base_pretrained(
        ...
        *,
        ...
        base_config = DreamConfig.from_pretrained(pretrained_model_name_or_path)
        history_config = DreamHistoryConfig.from_base_config(
            base_config,
...
def load_dream_history_model(model_args) -> DreamSyntheticRevisionHistoryModel:
    ...
    if _read_model_type(model_name_or_path) == DreamHistoryConfig.model_type:
        history_config = DreamHistoryConfig.from_pretrained(model_name_or_path)
        model = DreamSyntheticRevisionHistoryModel.from_pretrained(
            model_name_or_path,
            config=history_config,
            torch_dtype=dtype,
            device_map=_default_device_map(),
        ...
        model = DreamSyntheticRevisionHistoryModel.from_base_pretrained(
            model_name_or_path,
    ...
    if getattr(model_args, "lora", False):
        ...
        if modules_to_save:
            existing = [
                value.strip()
                for value in modules_to_save.split(",")
                if value.strip()
            ]
        else:
            existing = []
...
def resolve_dream_history_base_model_name_or_path(model_name_or_path: str) -> str:
...
def load_dream_history_tokenizer(model_args):
    base_model_name_or_path = resolve_dream_history_base_model_name_or_path(
...
try:
    AutoConfig.register(DreamHistoryConfig.model_type, DreamHistoryConfig)
except ValueError:
    pass
try:
    AutoModel.register(DreamHistoryConfig, DreamSyntheticRevisionHistoryModel)
except ValueError:
    pass
try:
